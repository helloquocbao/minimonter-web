...
export interface ZoneInfo {
  id: number;
  sponsor: string;
  /// Business name the sponsor paid to display. Untrusted, unverified text from whoever created
  /// the zone — nothing on-chain proves they are that business — so only ever render it as text.
  name: string;
  lat: number;
  lng: number;
  radiusMeters: number;
